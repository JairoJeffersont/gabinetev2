...
$objetivoGet = (isset($_GET['id'])) ? $_GET['id'] : null;

$buscaObjetivo = $objetivoEmendaController->buscar($objetivoGet);
        ...
        <div class="container-fluid p-2">
            <div class="card mb-2 ">
                <div class="card-body p-1">
                    ...
                    <a class="btn btn-success custom-card-body btn-sm link_loading" href="?secao=emendas-objetivos" role="button"><i class="bi bi-arrow-left"></i> Voltar</a>
                </div>
            </div>
            <div class="card mb-2">
                <div class="card-header bg-primary text-white px-2 py-1 custom-card-header"><i class="bi bi-flag"></i> Editar Objetivo da Emenda</div>
                <div class="card-body custom-card-body p-2">
                    ...
                    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['btn_salvar'])) {
                        ...
                        if ($buscaObjetivo['data']['gabinete'] == '1') {
                            ...
                            $result = $objetivoEmendaController->atualizar($objetivoGet, $dados);
                    ...
                    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['btn_apagar'])) {
                        ...
                        if ($buscaObjetivo['data']['gabinete'] == '1') {
                            ...
                            $result = $objetivoEmendaController->apagar($objetivoGet);
                            if ($result['status'] == 'success') {
                                header('Location: ?secao=emendas-objetivos');
                    ...
                    if ($_SESSION['tipo'] != '1' && $_SESSION['tipo'] != '5') {
                        echo '<div class="alert alert-danger custom-alert px-2 py-1 mb-2" role="alert">Você não tem autorização para inserir ou editar objetivos.</div>';
                    }

                    ?>
                    <form class="row g-2 form_custom" id="form_editar" method="POST">
                        <div class="col-md-4 col-12">
                            <input type="text" class="form-control form-control-sm" name="objetivo_nome" placeholder="Nome do Objetivo" value="<?= $buscaObjetivo['data']['nome'] ?>" required>
                        </div>
                        <div class="col-md-4 col-12">
                            <?php
                            if ($_SESSION['tipo'] == '1' || $_SESSION['tipo'] == '5') {
